...
proving to faculty that the pipeline handles scale, rapid updates, and
multiple companies gracefully.
...
BUDGET_MAP = {
    company: [str(uuid.uuid4()) for _ in range(BUDGETS_PER_COMPANY)]
...
def run_load_test():
    """Runs the load testing process by producing events to Kafka."""
    ...
    print(
        f"✅ Connected! Pumping {TOTAL_EVENTS_TO_GENERATE} events into '{KAFKA_TOPIC}'..."
    )

    total_sent = 0
    start_time = time.time()

    for _ in range(TOTAL_EVENTS_TO_GENERATE):
    ...
    print(
        "👉 Now check the Analysis Service logs or MongoDB Atlas to see them arrive instantly!"
    )
